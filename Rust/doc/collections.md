...
* `Set` любого варианта
  ...
  * Нет никакой значимой информации. которую можно связать с ключами.
  * Требуется набор.
* `BinaryHeap`
...
Важно. `Vec` обычно работает быстрее остальных. Так же `VecDeque` обычно быстрее `LinkedList`.
